...
import { UserPlus, Users, UserCheck, Search, MoreHorizontal, X, Check } from 'lucide-react'
...
export function FollowersPage() {
  ...
  const { isFollowing, followUser, unfollowUser, data } = useUserData()
      ...
      <div className="space-y-3">
        {filteredData.length === 0 ? (
          ...
          filteredData.map((user, index) => {
            const userIsFollowed = isFollowing(user.id) || (activeTab === 'following' && mockFollowingList.some(m => m.id === user.id))
            return (
              <motion.div
                key={user.id}
                initial={{ opacity: 0, y: 20 }}
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-medium text-white hover:text-primary-400 cursor-pointer transition-colors">
                      {user.username}
                    </p>
                    <p className="text-sm text-dark-400 line-clamp-1">{user.bio}</p>
                    <p className="text-xs text-dark-500">{user.followers.toLocaleString()} followers</p>
                  </div>
                </div>
